...
RSpec.describe Product, type: :model do
  describe 'Validations' do
    ...
    it 'Successfully saves a new product when all fields are provided' do
      @product = Product.new(name: "Smartwatch", price: 1_335.00, quantity: 11, category: @category)
      saved = @product.save
      expect(saved).to be_truthy
    end

    it 'returns an error message when the name field is missing' do
    ...
    it 'returns an error message when the price field is missing' do
      @product = Product.new(name: "Smartwatch", price: nil, quantity: 11, category: @category)
      saved = @product.save
      expect(@product.errors.full_messages ).to include("Price can't be blank")
    end

    it 'returns an error message when the quantity field is missing' do
      @product = Product.new(name: "Smartwatch", price: 1_335.00, quantity: nil, category: @category)
      saved = @product.save
      expect(@product.errors.full_messages).to include("Quantity can't be blank")
    end

    it 'returns an error message when the category field is missing' do
